...
bool PhotonAnalysis::SelectEventsReduction(LoopAll& l, int jentry)
{
    ...
    cout<<"presel_scet1 "<<presel_scet1<<endl;
    cout<<"presel_scet2 "<<presel_scet2<<endl;
...
//Added two category in Ht for hadronic, 3 cat for SUSY selection as checks
//Esiste anche una categoria Loose, ma se e' accesa il tag e' sempre true

int ISLeptonic=0;

//SUSY Cat
bool PhotonAnalysis::TprimeSusy_cat1(LoopAll& l, int& diphotonSusy_Cat1_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){

    if(isCiC==5){
        diphotonSusy_Cat1_id = l.DiphotonCiCSelection( l.phoSUPERTIGHT, l.phoSUPERTIGHT, 33,25, 4,false, &smeared_pho_energy[0], true);
    }

    cout<<"dentro Susy cat1"<<endl;
    bool tag = false;
    if(diphotonSusy_Cat1_id==-1) return tag;

    static std::vector<unsigned char> id_flags;
    if( jetid_flags == 0 ) {
        switchJetIdVertex( l, l.dipho_vtxind[diphotonSusy_Cat1_id] );
        id_flags.resize(l.jet_algoPF1_n);
        for(int ijet=0; ijet<l.jet_algoPF1_n; ++ijet ) {
            id_flags[ijet] = PileupJetIdentifier::passJetId(l.jet_algoPF1_cutbased_wp_level[ijet], PileupJetIdentifier::kLoose);
        }
    ...
    //////////////////Defining Susy_cat1 selection///////////////                                                                                                 
    float ptLead_thresh,ptSublead_thresh,ptLeadTrig_thresh,ptSubleadTrig_thresh;
    int nbJets_loose_thresh,nbJets_medium_thresh;
    float ptJets_thresh;

    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonSusy_Cat1_id], l.dipho_vtxind[diphotonSusy_Cat1_id], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonSusy_Cat1_id], l.dipho_vtxind[diphotonSusy_Cat1_id], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    //photon cuts
    ptLead_thresh=40; //not running for SUSY
    ptSublead_thresh=25.;//not running for SUSY
    //ptLeadTrig_thresh=33.;
    //ptSubleadTrig_thresh=25.;
    ...
    for(int ii=0; ii<l.jet_algoPF1_n; ++ii) {
        ...
        if(l.jet_algoPF1_csvBtag[ii]>0.244){njets_btagloose++; loose_index=ii;}// loose_index needed for invariant mass
        ...
        bool hasPassedPhotonSelection= (lead_p4.Pt()> ptLead_thresh && lead_p4.Eta()<1.4442 && sublead_p4.Pt()>ptSublead_thresh && sublead_p4.Eta()<1.4442);//susy=> both photons in barrel
...
bool PhotonAnalysis::TprimeSusy_cat2(LoopAll& l, int& diphotonSusy_Cat2_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
    ...
    static std::vector<unsigned char> id_flags;
    if( jetid_flags == 0 ) {
        switchJetIdVertex( l, l.dipho_vtxind[diphotonSusy_Cat2_id] );
        id_flags.resize(l.jet_algoPF1_n);
        for(int ijet=0; ijet<l.jet_algoPF1_n; ++ijet ) {
            id_flags[ijet] = PileupJetIdentifier::passJetId(l.jet_algoPF1_cutbased_wp_level[ijet], PileupJetIdentifier::kLoose);
        }
    ...
    //////////////////Defining Susy_cat2 selection///////////////                                                                                                 
    float ptLead_thresh,ptSublead_thresh,ptLeadTrig_thresh,ptSubleadTrig_thresh;
    int nbJets_loose_thresh,nbJets_medium_thresh;
    float ptJets_thresh;

    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonSusy_Cat2_id], l.dipho_vtxind[diphotonSusy_Cat2_id], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonSusy_Cat2_id], l.dipho_vtxind[diphotonSusy_Cat2_id], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    for(int ii=0; ii<l.jet_algoPF1_n; ++ii) {
        ...
        if(hasPassedJetSelection && hasPassedPhotonSelection && invbMass>95 && invbMass<155){tag=true;}
        if(tag==true){cout<< "tag true per SUSY cat 2"<<endl;}
        return tag;
    }

bool PhotonAnalysis::TprimeSusy_cat3(LoopAll& l, int& diphotonSusy_Cat3_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){

    if(isCiC==5){
        diphotonSusy_Cat3_id = l.DiphotonCiCSelection( l.phoSUPERTIGHT, l.phoSUPERTIGHT, 33,25, 4,false, &smeared_pho_energy[0], true);
    }

    cout<<"dentro Susy cat1"<<endl;
    bool tag = false;
    if(diphotonSusy_Cat3_id==-1) return tag;

    static std::vector<unsigned char> id_flags;
    if( jetid_flags == 0 ) {
        switchJetIdVertex( l, l.dipho_vtxind[diphotonSusy_Cat3_id] );
        id_flags.resize(l.jet_algoPF1_n);
        for(int ijet=0; ijet<l.jet_algoPF1_n; ++ijet ) {
            id_flags[ijet] = PileupJetIdentifier::passJetId(l.jet_algoPF1_cutbased_wp_level[ijet], PileupJetIdentifier::kLoose);
        }
    ...
    //////////////////Defining Susy_cat3 selection///////////////                                                                                                 
    float ptLead_thresh,ptSublead_thresh,ptLeadTrig_thresh,ptSubleadTrig_thresh;
    int nbJets_loose_thresh,nbJets_medium_thresh;
    float ptJets_thresh;

    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonSusy_Cat3_id], l.dipho_vtxind[diphotonSusy_Cat3_id], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonSusy_Cat3_id], l.dipho_vtxind[diphotonSusy_Cat3_id], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    for(int ii=0; ii<l.jet_algoPF1_n; ++ii) {
        ...
        if(tag==true){cout<< "tag true per SUSY cat 3"<<endl;}
        return tag;
    }

bool PhotonAnalysis::TprimehadronicTag2012_lowHt(LoopAll& l, int& diphotonTprimehad_id_lowHt, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
    cout<<"*********************"<<endl;
    //giuseppe
    //    if(!mvaselection){CASO MIO
    if(isCiC==5){
        diphotonTprimehad_id_lowHt = l.DiphotonCiCSelection( l.phoSUPERTIGHT, l.phoSUPERTIGHT, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==4){
        diphotonTprimehad_id_lowHt = l.DiphotonCiCSelection( l.phoTIGHT, l.phoTIGHT, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==3){
        cout<<"USING CIC MEDIUM"<<endl;
        diphotonTprimehad_id_lowHt = l.DiphotonCiCSelection( l.phoMEDIUM, l.phoMEDIUM, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==2){
        diphotonTprimehad_id_lowHt = l.DiphotonCiCSelection( l.phoLOOSE, l.phoLOOSE, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==1){
        diphotonTprimehad_id_lowHt = l.DiphotonCiCSelection( l.phoNOCUTS, l.phoNOCUTS, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }

        cout<<"dentro Tprime hadronic tag low Ht"<<endl;
        cout<<"leadEtTprimehadCut"<<leadEtTprimehadCut<<endl;
        cout<<"subleadEtTprimehadCut"<<subleadEtTprimehadCut<<endl;
    bool tag = false;
    if(diphotonTprimehad_id_lowHt==-1) return tag;//e' stato calcolato prima, si puo togliere
    cout<<"diphotonTprimehad!=-1!"<<endl;
    static std::vector<unsigned char> id_flags;
    if( jetid_flags == 0 ) {
        switchJetIdVertex( l, l.dipho_vtxind[diphotonTprimehad_id_lowHt] );
        id_flags.resize(l.jet_algoPF1_n);
        for(int ijet=0; ijet<l.jet_algoPF1_n; ++ijet ) {
            id_flags[ijet] = PileupJetIdentifier::passJetId(l.jet_algoPF1_cutbased_wp_level[ijet], PileupJetIdentifier::kLoose);
        }
    ...
    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonTprimehad_id_lowHt], l.dipho_vtxind[diphotonTprimehad_id_lowHt], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonTprimehad_id_lowHt], l.dipho_vtxind[diphotonTprimehad_id_lowHt], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    //photon cuts
    ptLead_thresh=90*diphoton.M()/120.;
    ptSublead_thresh=35.;
    ptLeadTrig_thresh=33.;
    ptSubleadTrig_thresh=25.;

    //jet cuts                                                                                                                                               
    nJets_thresh=2;//Preselection
    ptJets_thresh=25;
    //Ht_thresh=1000;=> tra 800 e 1500
    nbJets_thresh=1;//loose per adronica
    ...
    for(int ii=0; ii<l.jet_algoPF1_n; ++ii) {
        ...
        bool hasPassedJetSelection= (njets>=nJets_thresh && njets_btagloose>=nbJets_thresh && Ht > 800 && Ht<1500);//REAL selection                                
        ...
        bool hasPassedJetPreSelection = (njets>=nJets_thresh);
        bool hasPassedPhotonPreSelection = (lead_p4.Pt()>ptLeadTrig_thresh && sublead_p4.Pt()> ptSubleadTrig_thresh);
        ...
        cout<<"hasPassedPhotonSelection "<<hasPassedPhotonSelection<<endl;
        cout<<"hasPassedJetSelection && hasPassedPhotonSelection "<<(hasPassedJetSelection && hasPassedPhotonSelection)<<endl;
        if(hasPassedJetSelection && hasPassedPhotonSelection){tag=true;}//Commenta-scommenta per eff_presel e spegni Tprime_lep
        //        if(hasPassedJetPreSelection && hasPassedPhotonPreSelection){tag=true;}//Commenta-scommenta per eff_presel e spegni Tprime_lep in analysis_settings_Tprime.dat e in statanalysis.dat

        if(tag==true){cout<< "tag true per Tprime had low Ht"<<endl;}
        if (PADEBUG && tag==true) cout<<"tagged Tprime had low Ht"<<endl;
        return tag;
    }

bool PhotonAnalysis::TprimehadronicTag2012_highHt(LoopAll& l, int& diphotonTprimehad_id_highHt, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
    //giuseppe
    //    if(!mvaselection){CASO MIO
    if(isCiC==5){
        diphotonTprimehad_id_highHt = l.DiphotonCiCSelection( l.phoSUPERTIGHT, l.phoSUPERTIGHT, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==4){
        diphotonTprimehad_id_highHt = l.DiphotonCiCSelection( l.phoTIGHT, l.phoTIGHT, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==3){
        cout<<"USING CIC MEDIUM"<<endl;
        diphotonTprimehad_id_highHt = l.DiphotonCiCSelection( l.phoMEDIUM, l.phoMEDIUM, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==2){
        diphotonTprimehad_id_highHt = l.DiphotonCiCSelection( l.phoLOOSE, l.phoLOOSE, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==1){
        diphotonTprimehad_id_highHt = l.DiphotonCiCSelection( l.phoNOCUTS, l.phoNOCUTS, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }
        ...
        cout<<"subleadEtTprimehadCut"<<subleadEtTprimehadCut<<endl;
    bool tag = false;
    if(diphotonTprimehad_id_highHt==-1) return tag;//e' stato calcolato prima, si puo togliere
    cout<<"diphotonTprimehad!=-1!"<<endl;
    static std::vector<unsigned char> id_flags;
    if( jetid_flags == 0 ) {
        switchJetIdVertex( l, l.dipho_vtxind[diphotonTprimehad_id_highHt] );
        id_flags.resize(l.jet_algoPF1_n);
        for(int ijet=0; ijet<l.jet_algoPF1_n; ++ijet ) {
            id_flags[ijet] = PileupJetIdentifier::passJetId(l.jet_algoPF1_cutbased_wp_level[ijet], PileupJetIdentifier::kLoose);
        }
    ...
    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonTprimehad_id_highHt], l.dipho_vtxind[diphotonTprimehad_id_highHt], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonTprimehad_id_highHt], l.dipho_vtxind[diphotonTprimehad_id_highHt], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    nJets_thresh=2;//Preselection
    ptJets_thresh=25;
    Ht_thresh=1500;//test for adding category in Ht
    nbJets_thresh=1;//loose per adronica
    ...
    for(int ii=0; ii<l.jet_algoPF1_n; ++ii) {
        ...
        if(tag==true){cout<< "tag true per Tprime had high Ht"<<endl;}
        if (PADEBUG && tag==true) cout<<"tagged Tprime had high Ht"<<endl;
        return tag;
    }

////Category hadronic, without splitting in Ht

bool PhotonAnalysis::TprimehadronicTag2012(LoopAll& l, int& diphotonTprimehad_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
    //giuseppe
    //    if(!mvaselection){
    if(isCiC==5){
        ...
        diphotonTprimehad_id = l.DiphotonCiCSelection( l.phoTIGHT, l.phoTIGHT, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==3){
        cout<<"USING CIC MEDIUM"<<endl;
        diphotonTprimehad_id = l.DiphotonCiCSelection( l.phoMEDIUM, l.phoMEDIUM, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==2){
        diphotonTprimehad_id = l.DiphotonCiCSelection( l.phoLOOSE, l.phoLOOSE, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }else if(isCiC==1){
        diphotonTprimehad_id = l.DiphotonCiCSelection( l.phoNOCUTS, l.phoNOCUTS, leadEtTprimehadCut,subleadEtTprimehadCut, 4,false, &smeared_pho_energy[0], true);
    }
    ...
    //defining Tprime variables                                                                                                                              
    TLorentzVector lead_p4 = l.get_pho_p4( l.dipho_leadind[diphotonTprimehad_id], l.dipho_vtxind[diphotonTprimehad_id], &smeared_pho_energy[0]);
    TLorentzVector sublead_p4 = l.get_pho_p4( l.dipho_subleadind[diphotonTprimehad_id], l.dipho_vtxind[diphotonTprimehad_id], &smeared_pho_energy[0]);
    TLorentzVector diphoton = lead_p4+sublead_p4;
    ...
    bool PhotonAnalysis::TprimeleptonicTag2012(LoopAll& l, int& diphotonTprimelep_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
        ...
        //    if(!mvaselection){
        if(isCiC==5){
        diphotonTprimelep_id = l.DiphotonCiCSelection( l.phoSUPERTIGHT, l.phoSUPERTIGHT, leadEtTprimelepCut,subleadEtTprimelepCut, 4,false, &smeared_pho_energy[0], true, -1, veto_indices);
        }else if(isCiC==3){
            cout<<"USING CIC MEDIUM"<<endl;
        diphotonTprimelep_id = l.DiphotonCiCSelection( l.phoMEDIUM, l.phoMEDIUM, leadEtTprimelepCut,subleadEtTprimelepCut, 4,false, &smeared_pho_energy[0], true, -1, veto_indices);
        }
    ...
    float ptLead_thresh,ptSublead_thresh,ptLeadTrig_thresh,ptSubleadTrig_thresh;
        ...
        Ht+=lead_p4.Pt() + sublead_p4.Pt()+lep->Pt();
        ...
        //doing the selection                                                                                                                                    
        bool hasPassedJetSelection= (njets>=nJets_thresh && njets_btagmedium>=nbJets_thresh && Ht > Ht_thresh);
        bool hasPassedPhotonSelection= (lead_p4.Pt()>ptLeadTrig_thresh && sublead_p4.Pt()> ptSubleadTrig_thresh && lead_p4.Pt()> ptLead_thresh && sublead_p4.Pt()>ptSublead_thresh);
        bool hasPassedPreselection= (njets>=2 && lead_p4.Pt()>33. && sublead_p4.Pt()>25.);

        //if(hasPassedJetSelection && hasPassedPhotonSelection)tag=true;
        if(hasPassedPreselection){tag=true; ISLeptonic=1;}//use this in order to calculate preselection efficiencies
...
bool PhotonAnalysis::TTHleptonicTag2012(LoopAll& l, int& diphotonTTHlep_id, float* smeared_pho_energy, bool *jetid_flags, bool mvaselection,bool vetodipho,bool kinonly){
    ...
    */
    ...
    cout<<"Alla fine di tth lep"<<endl;
    if(hasPassedJetSelection && hasPassedPhotonSelection)tag=true;
...
void PhotonAnalysis::saveDatCardTree(LoopAll &l, int cur_type, int category, int inc_cat, float evweight, int ipho1, int ipho2, int ivtx, TLorentzVector lead_p4, TLorentzVector sublead_p4, bool isCutBased, string proc, double sigmaMrv, double sigmaMwv, double sigmaMeonly, float vtxP, string trainPhil, string bdtType, float lead_id_mva, float sublead_id_mva){
   ...
   // track the scale and smear uncertainties per event
    ...
    if (proc==Form("THWB_mass_m%3.0f",l.normalizer()->GetMass(cur_type))) proc_id=5;//Giuseppe: diverso da prima
    if (proc==Form("THtZ_mass_m%3.0f",l.normalizer()->GetMass(cur_type))) proc_id=6;//Giuseppe: diverso da prima
    //    if (proc==Form("wzh_mass_m%3.0f",l.normalizer()->GetMass(cur_type))) proc_id=5; //GIUSEPPE
