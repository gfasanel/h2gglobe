...
void makeBkgPlotsGeneric_my(std::string filebkg, std::string filesig="", bool blind=true, bool doBands=true, bool baseline=true, bool useBinnedData=false){
	...
	gStyle->SetOptStat(0);
	//	const int ncats = 9;
	const int ncats = 8;//mio; 4 inclusive + 2 ttH + 2 Tprime
	...
	std::string massfactlabels[9] = { //Io non uso MVA
	...
	//RooWorkspace *w_bkgGiuseppe  = (RooWorkspace*) fbGiuseppe->Get("newWs");
//	w_bkg->Print();
